...
import numpy as np
from sparx import get_image, binarize, filt_gaussl, model_square, model_circle
...
def excise_window_and_mask(map, kji, wn, mask):
    k, j, i = kji
...
def get_central_scaled_pixel_vals_after_scaling(emmap, modmap, masked_xyz_locs, wn, apix):
    ...
    emmap = np.copy(EMNumPy.em2numpy(emmap))
    modmap = np.copy(EMNumPy.em2numpy(modmap))
    
    falloff_width = 0.1 * wn / 2.0
    sphere_dim = int(2 * falloff_width + wn / 2.0)
    mask = model_circle(int(round(sphere_dim / 2.0)), wn, wn, wn)
    mask = filt_gaussl(mask, 0.44 / falloff_width)
    central_pix = int(round(wn / 2.0))
    for kji in masked_xyz_locs:
        emmap_wn = excise_window_and_mask(emmap, kji, wn, mask)
        modmap_wn = excise_window_and_mask(modmap, kji, wn, mask)
...
def prepare_mask_and_maps_for_scaling(args):
    ...
    if args.window_size is None:
        wn = padded_size * int(round(7 * 3 * args.apix)) # set default window size to 7 times average resolution
